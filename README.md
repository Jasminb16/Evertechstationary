...
Phone, WhatsApp, email, address, opening hours, social links, headline stats.
Every appearance of these across the site reads from here, so changing the
phone number is a one-line edit.

> **⚠️ Contains placeholders.** Phone, WhatsApp, email and address are dummy
> values marked `// TODO`. The `stats` block (`2,500+ products`,
> `500+ businesses`) are **invented placeholder claims about the business** —
> replace them with real figures or delete them before launch.
...
await resend.emails.send({
  ...
  to: "sales@evertechstationery.com",
  replyTo: body.email,
...
- [ ] Replace every `// TODO` in `src/lib/site.ts` — phone, WhatsApp, email, address
...
- [ ] Add real social URLs (they currently point at the platform home pages)
- [ ] Update `site.address.mapQuery` so the contact-page map shows the real pin
